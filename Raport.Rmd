...
for (i in colnames(sledzie)) {
  ...
  if(i=="cfin1"){
    title <- "Dostêpnoœæ planktonu (Calanus finmarchicus : gat 1)"
  }
  if(i=="cfin2"){
    title <- "Dostêpnoœæ planktonu (Calanus finmarchicus : gat 2)"
  }
  if(i=="chel1"){
    title <- "Dostêpnoœæ planktonu (Calanus helgolandicus  : gat 1)"
  }
  if(i=="chel2"){
    title <- "Dostêpnoœæ planktonu (Calanus helgolandicus  : gat 2)"
  }
  if(i=="lcop1"){
    title <- "Dostêpnoœæ planktonu (Zagêszczenie wid³onogów : gat 1)"
  }
  if(i=="lcop2"){
    title <- "Dostêpnoœæ planktonu (Zagêszczenie wid³onogów : gat 2"
  }
   if(i=="fbar"){
    title <- "Natê¿enie po³owów w regionie (u³amek pozostawionego narybku)"
   }
   if(i=="recr"){
    title <- "Roczny narybek (liczba œledzi)"
   }
   if(i=="cumf"){
    title <- "£¹czne roczne natê¿enie po³owów w regionie (u³amek pozostawionego narybku)"
   }
   if(i=="totaln"){
    title <- "£¹czna liczba ryb z³owionych w ramach po³owu (liczba œledzi)"
   }
   if(i=="sst"){
   ...
   if(i=="sal"){
    ...
    if(i=="nao"){
      title <- "Oscylacja pó³nocnoatlantycka (mb)"
    }
    if(title!=""){
        print(
          ggplot(data = sledzie,
